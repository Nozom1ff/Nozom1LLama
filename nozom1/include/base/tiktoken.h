                             ...
                             std::function<int(int, int)> func) -> std::vector<int>
{
    ...
    auto get_rank = [&piece, &ranks](
                        const std::vector<std::pair<int, int>> &parts, int start_idx, int skip) -> std::optional<int> {
...
static auto byte_pair_encode(const std::string &piece, const ankerl::unordered_dense::map<std::string, int> &ranks)
    -> std::vector<int>
{
    if (piece.size() == 1)
    {
...
public:
             ...
             const std::string &pattern)
    {
        ...
        // assert(encoder_.size() != decoder_.size() && "Encoder and decoder must be of equal length;
