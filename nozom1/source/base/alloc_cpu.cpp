...
#    define NOZOM1_HAVE_POSIX_MEMALIGN
#endif

namespace base
{
CPUDeviceAllocator::CPUDeviceAllocator() : DeviceAllocator(DeviceType::kCPU) {}
...
#ifdef NOZOM1_HAVE_POSIX_MEMALIGN
    void *data             = nullptr;
    const size_t alignment = (byte_size >= size_t(1024)) ? size_t(32) : size_t(16);
    int status =
        posix_memalign((void **)&data, ((alignment >= sizeof(void *))) ? alignment : sizeof(void *), byte_size);
    if (status != 0)
        return nullptr;
    return data;
#else
