#include "base/buffer.h"
#include <glog/logging.h>

namespace base
{

Buffer::Buffer(size_t byte_size, std::shared_ptr<DeviceAllocator> allocator, void *ptr, bool use_external)
    : byte_size_(byte_size),
      ptr_(ptr),
      use_external_(use_external),
      allocator_(allocator)
{
    if (!ptr_ && allocator_)
    {
        device_type_  = allocator_->deviceType();
        use_external_ = false;
        ptr_          = allocator_->allocate(byte_size);
    }
}

Buffer::~Buffer()
{
    if (!use_external_)
    {
        if (ptr_ && allocator_)
        {
            allocator_->release(ptr_);
            ptr_ = nullptr;
        }
    }
}

void *Buffer::ptr()
{
    return ptr_;
}

const void *Buffer::ptr() const
{
    return ptr_;
}

size_t Buffer::byte_size() const
{
    return byte_size_;
}

bool Buffer::allocate()
{  // NOTE allocator好像只有初始化的时候可以给啊
    if (allocator_ && byte_size_ != 0)
    {
        use_external_ = false;
        ptr_          = allocator_->allocate(byte_size_);
        return ptr_ != nullptr;
    }
    return false;
}

std::shared_ptr<DeviceAllocator> Buffer::allocator() const
{
    return allocator_;
}

DeviceType Buffer::device_type() const
{
    return device_type_;
}

void Buffer::set_device_type(DeviceType device_type)
{
    device_type_ = device_type;
}

std::shared_ptr<Buffer> Buffer::get_shared_from_this()
{
    return shared_from_this();
}

bool Buffer::is_external() const
{
    return this->use_external_;
}

void Buffer::copy_from(const Buffer &buffer) const
{
    CHECK(allocator_ != nullptr);
    CHECK(buffer.ptr_ != nullptr);

    size_t byte_size              = byte_size_ < buffer.byte_size_ ? byte_size_ : buffer.byte_size_;
    const DeviceType &src_devcice = buffer.device_type();
    const DeviceType &dst_device  = this->device_type();
    CHECK(src_devcice != DeviceType::kUnknown && dst_device != DeviceType::kUnknown);

    if (src_devcice == DeviceType::kCPU && dst_device == DeviceType::kCPU)
    {
        return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size);
    }
    else if (src_devcice == DeviceType::kCUDA && dst_device == DeviceType::kCPU)
    {
        return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size, MemcpyKind::kMemcpyCUDA2CPU);
    }
    else if (src_devcice == DeviceType::kCPU && dst_device == DeviceType::kCUDA)
    {
        return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size, MemcpyKind::kMemcpyCPU2CUDA);
    }
    else
    {
        return allocator_->memcpy(buffer.ptr(), this->ptr_, byte_size, MemcpyKind::kMemcpyCUDA2CUDA);
    }
};

void Buffer::copy_from(const Buffer *buffer) const
{
    // NOTE 这意思是允许自己可以空 拷贝源不能空
    CHECK(allocator_ != nullptr);
    CHECK(buffer != nullptr || buffer->ptr_ != nullptr);

    size_t dst_size  = byte_size_;
    size_t src_size  = buffer->byte_size_;
    size_t byte_size = src_size < dst_size ? src_size : dst_size;

    const DeviceType &src_device = buffer->device_type();
    const DeviceType &dst_device = this->device_type();
    CHECK(src_device != DeviceType::kUnknown && dst_device != DeviceType::kUnknown);

    if (src_device == DeviceType::kCPU && dst_device == DeviceType::kCPU)
    {
        return allocator_->memcpy(buffer->ptr_, this->ptr_, byte_size);
    }
    else if (src_device == DeviceType::kCUDA && dst_device == DeviceType::kCPU)
    {
        return allocator_->memcpy(buffer->ptr_, this->ptr_, byte_size, MemcpyKind::kMemcpyCUDA2CPU);
    }
    else if (src_device == DeviceType::kCPU && dst_device == DeviceType::kCUDA)
    {
        return allocator_->memcpy(buffer->ptr_, this->ptr_, byte_size, MemcpyKind::kMemcpyCPU2CUDA);
    }
    else
    {
        return allocator_->memcpy(buffer->ptr_, this->ptr_, byte_size, MemcpyKind::kMemcpyCUDA2CUDA);
    }
}
}  // namespace base