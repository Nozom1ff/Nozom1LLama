...
{
    return base::DataTypeSize(data_type);
}
...
Tensor::Tensor(base::DataType data_type,
               ...
               std::shared_ptr<base::DeviceAllocator> alloc,
               void *ptr)
    : Tensor(data_type, std::vector<int32_t>{dim0}, need_alloc, alloc, ptr)
{
    // 委托给主构造函数，所有初始化由主构造函数完成
}
...
Tensor::Tensor(base::DataType data_type,
               ...
               std::shared_ptr<base::DeviceAllocator> alloc,
               void *ptr)
    : Tensor(data_type, std::vector<int32_t>{dim0, dim1}, need_alloc, alloc, ptr)
{
    // 委托给主构造函数，所有初始化由主构造函数完成
}
...
Tensor::Tensor(base::DataType data_type,
               ...
               std::shared_ptr<base::DeviceAllocator> alloc,
               void *ptr)
    : Tensor(data_type, std::vector<int32_t>{dim0, dim1, dim2}, need_alloc, alloc, ptr)
{
    // 委托给主构造函数，所有初始化由主构造函数完成
}
...
Tensor::Tensor(base::DataType data_type,
               ...
               std::shared_ptr<base::DeviceAllocator> alloc,
               void *ptr)
    : Tensor(data_type, std::vector<int32_t>{dim0, dim1, dim2, dim3}, need_alloc, alloc, ptr)
{
    // 委托给主构造函数，所有初始化由主构造函数完成
}
...
void Tensor::to_cuda(cudaStream_t stream)
{
    ...
    else if (device_type == base::DeviceType::kCPU)
    {
