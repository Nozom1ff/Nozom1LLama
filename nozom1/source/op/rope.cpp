...
namespace op
{
RoPELayer::RoPELayer(base::DeviceType device_type,
                     int32_t dim,
                     int32_t kv_dim,
                     int32_t head_size,
                     base::DataType data_type = base::DataType::kTypeFp32)
    : Layer(device_type, LayerType::kLayerRoPe, data_type, "RoPe"),
      dim_(dim),
      kv_dim_(kv_dim),
...
base::Status RoPELayer::forward()
{
    ...
    kernel::get_rope_kernel(device_type_)(dim_,
                                          kv_dim_,
                                          head_size_,
                                          input_q,
                                          input_k,
...
{
    // pos tensor
    auto status = check_tensor_with_dim(get_input(2), base::DeviceType::kCPU, base::DataType::kTypeInt32, 1);
    if (!status)
    {
