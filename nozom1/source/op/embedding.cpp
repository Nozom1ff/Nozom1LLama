...
namespace op
{
//[x] layer param 也加入了data_type形参
EmbeddingLayer::EmbeddingLayer(base::DeviceType device_type,
                               int32_t dim,
                               int32_t seq_len,
                               int32_t vocab_size,
...
base::Status EmbeddingLayer::forward()
{
    ...
    kernel::get_emb_kernel(device_type_)(
        get_input(0), get_weight(0), get_output(0), vocab_size_, cuda_config_ ? cuda_config_->stream : nullptr);
