...
MatmulLayer::MatmulLayer(base::DeviceType device_type,
    ...
    : LayerParam(device_type, LayerType::kLayerMatmul, data_type, is_quant_layer, "Matmul"),
      dim0_(dim0),
      dim1_(dim1),
      has_bias_(has_bias)
{
...
base::Status MatmulLayer::check() const
{
    ...
    {
        status = check_tensor_with_dim(get_weight(0), device_type_, base::DataType::kTypeInt8, dim0_, dim1_);
        if (!status)
        {
            LOG(ERROR) << "The weight tensor error in the matmul layer.";
            return status;
    ...
    if (is_quant_layer_)
    {
        status = check_tensor_with_dim(scales_, device_type_, base::DataType::kTypeFp32, scales_.size());
        if (!status)
        {
...
base::Status MatmulLayer::forward()
{
    ...
    if (is_quant_layer_)
    {
        kernel::get_matmul_kernel_quant8(device_type_)(get_input(0),
                                                       get_weight(0),
                                                       get_output(0),
                                                       group_size_,
                                                       scales_,
    ...
    else
    {
        kernel::get_matmul_kernel(device_type_)(
            get_input(0), get_weight(0), get_output(0), 1.f, cuda_config_ ? cuda_config_.get() : nullptr);
    }

    if (has_bias_)
    {
        kernel::get_add_kernel(device_type_)(
            get_output(0), get_bias(0), get_output(0), cuda_config_ ? cuda_config_->stream : nullptr);
    }
    return base::error::Success();
}

// NOTE 这里一个一个地set bias啊？这里idx就是0 硬编码了
base::Status MatmulLayer::set_bias(int32_t idx, int32_t &dim, const void *bias_ptr, base::DeviceType device_type)
{
    ...
    if (!is_quant_layer_)
    {
        tensor::Tensor bias(base::DataType::kTypeFp32, dim);
    ...
    {
        tensor::Tensor bias(base::DataType::kTypeInt8, dim);
        bias.set_device_type(device_type);
        CHECK(bias.assign(buffer));
        bias_.at(idx)           = bias;
        ...
        scales_ = tensor::Tensor{base::DataType::kTypeFp32,
                                 scale_nums,
                                 false,
                                 nullptr,
