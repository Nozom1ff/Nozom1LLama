...
__global__ void rope_kernel_cu_fp32(int pos,
                                    int dim,
                                    int kv_dim,
                                    int head_size,
                                    const float *input_q,
...
#else
__device__ void rope_calc(float cos_v, float sin_v, float *vec, int32_t idx)
{
    float2 *vec_ptr  = reinterpret_cast<float2 *>(vec + idx);
    float2 vec_value = *vec_ptr;
    *vec_ptr = make_float2(vec_value.x * cos_v - vec_value.x * sin_v, vec_value.y * sin_v + vec_value.y * cos_v);
}
                                    ...
                                    const float *cos_cache)
{
    ...
    int cur_theta = idx % head_size;
    float sin_v   = *(sin_cache + pos * head_size + cur_theta);
    float cos_v   = *(cos_cache + pos * head_size + cur_theta);

    rope_calc(cos_v, sin_v, const_cast<float *>(input_q), idx);
    if (idx >= kv_dim)
    {
        return;
    }
    rope_calc(cos_v, sin_v, const_cast<float *>(input_k), idx);
}

/**
 * @brief 缓存sin cos 这是一个很好的做法
 * @note 这里只用一个block
 */
__global__ void sin_cos_calc(int head_size, int max_seq_len, float *sin_cache, float *cos_cache)
{
    int idx = threadIdx.x + blockDim.x * blockIdx.x;
    // NOTE 所以head_size == threads!

    int cur_theta    = idx % head_size;
    const float freq = 1.0f / pow(10000.0f, static_cast<float>(cur_theta) / static_cast<float>(head_size));
    for (int pos = 0; pos < max_seq_len; ++pos)
    {
        float val   = static_cast<float>(pos) * freq;
        float cos_v = cosf(val);
        float sin_v = sinf(val);
        // NOTE 每个thread处理一个token的一个位置 ，循环max_seq_len次，所以步长为token_size
        *(cos_cache + pos * head_size + cur_theta) = cos_v;
        *(sin_cache + pos * head_size + cur_theta) = sin_v;
    }
}
#endif
                           ...
                           cudaStream_t stream)
{
    ...
    if (stream)
    {
        sin_cos_calc<<<1, threads, 0, stream>>>(head_size,
                                                max_seq_len,
    ...
    {
        sin_cos_calc<<<1, threads>>>(head_size,
                                     max_seq_len,
                                     const_cast<float *>(sin_cache.ptr<float>()),
                                     const_cast<float *>(cos_cache.ptr<float>()));
    }
...
// NOTE cos_cache和sin_cache都是外面分配的啊
void rope_kernel_cu(int32_t dim,
                    int32_t kv_dim,
                    int32_t head_size,
                    ...
                    void *stream)
{
    ...
    if (stream)
    {
        ...
        rope_kernel_cu_fp32<<<blocks, threads, 0, stream_>>>(pos,
                                                             dim,
                                                             kv_dim,
                                                             head_size,
                                                             input_q.ptr<float>(),
    ...
    {
        rope_kernel_cu_fp32<<<blocks, threads>>>(pos,
                                                 dim,
                                                 kv_dim,
                                                 head_size,
                                                 input_q.ptr<float>(),
